...
class ThinkFormat(ORM):
    def __call__(self, completions, **kwargs) -> List[float]:
        # 1. 必须加 () 捕获中间内容，否则无法判断“不为空”
        results = []
        for content in completions:
            # 提取所有配对成功的中间内容
            inner_matches = re.findall(r'<think>(.*?)</think>', content, re.DOTALL)
            # 2. 显式统计字面量数量（防嵌套/错位漏判）
            open_cnt = content.count('<think>')
            close_cnt = content.count('</think>')

            # 必须同时满足 4 个条件
            if (len(inner_matches) == 1 and  # 正则配对仅 1 次
                    open_cnt == 1 and  # 开标签仅 1 次
                    close_cnt == 1 and  # 闭标签仅 1 次
                    inner_matches[0].strip()):  # 中间内容非空（过滤纯空格/换行）
                results.append(1.0)
            else:
                results.append(0.0)
        return results
orms['thinkformat'] = ThinkFormat

class AsyncUltrasoundGenRMReward(AsyncORM):
    """
    多模态超声医学图像生成式奖励模型
    根据用户问题、超声图像和AI回答，评估回答的质量和准确性
    """

    def __init__(self, args, **kwargs):
        super().__init__(args)
        self.api_base = os.getenv('GENRM_API_BASE', 'http://localhost:7000/v1')
        self.temperature = float(os.getenv('GENRM_TEMPERATURE', '0.3'))
        self.max_tokens = int(os.getenv('GENRM_MAX_TOKENS', '2048'))
        ...
        try:
            ...
            self.model_name = self.client.models.list().data[0].id
            logger.info(f'AsyncUltrasoundGenRMReward initialized with model: {self.model_name}')
        except Exception as e:
            raise RuntimeError('Failed to connect to the model service. Please deploy the model '
                               "using 'swift deploy --model <multimodal_model_name> --port 8000 --infer_backend vllm'.") from e

        # 超声医学评估系统提示词（在原基础上最小修改，加入超声专业内容）
        self.system_prompt = textwrap.dedent("""
            You are an expert ultrasound radiologist evaluator. Your task is to evaluate the quality of an AI assistant's response to a question about an ultrasound medical image.

            Please evaluate the response based on the following criteria:
            1. Correctness: Is the answer factually correct, logically sound, and accurately describes the findings in the ultrasound image?
            2. Helpfulness: Does the response address the user's question effectively and provide relevant ultrasound-specific information?
            3. Clarity: Is the response well-organized, easy to understand, and uses appropriate medical terminology?
            4. Completeness: Does the response cover all key ultrasound findings mentioned in the question or visible in the image?

            After your evaluation, provide a score from 0 to 10, where:
            - 0-3: Poor quality (incorrect, unhelpful, confusing, or completely misses key ultrasound findings)
            - 4-6: Acceptable quality (partially correct or helpful, misses some important details)
            - 7-9: Good quality (correct, helpful, clear, and accurately describes most ultrasound findings)
            - 10: Excellent quality (perfect response, comprehensive and precise ultrasound interpretation)

            You MUST end your response with the score in this exact format: [[score]]
            For example: [[7]] or [[10]]
        """).strip()

    def _is_base64_image(self, image_str: str) -> bool:
        """检查字符串是否为base64编码的图像"""
        try:
            if image_str.startswith('data:image/'):
                return True
            # 尝试解码前几个字符验证格式
            base64.b64decode(image_str[:100], validate=True)
            return True
        except:
            return False

    def _build_multimodal_message(self, question: str, image: Union[str, List[str]]) -> List[dict]:
        """构建包含文本和图像的多模态消息内容"""
        content = [{"type": "text",
                    "text": f"## User Question\n{question}\n\n## Ultrasound Image Analysis\nPlease evaluate the AI response based on the provided ultrasound image(s) and question."}]
    ...
    def _build_eval_prompt(self, completion: str) -> str:
        """构建评估提示词（文本部分）"""
        return textwrap.dedent(f"""
            ## AI Assistant's Response
            {completion}
            ...
            ## Your Evaluation
            Please evaluate the above response based on the ultrasound image(s) and question, and provide your score.
        """).strip()

    def _extract_score(self, response: str) -> float:
        """从奖励模型的响应中提取分数（保持原逻辑不变）"""
        # 查找 [[score]] 模式
        match = re.search(r'\[\[(\d+(?:\.\d+)?)\]\]', response)
        if match:
            score = float(match.group(1))
    ...
    async def _score_single(self, session, question: str, image: Union[str, List[str]], completion: str) -> float:
        """使用多模态生成式奖励模型评估单个回答"""
        import aiohttp

        # 构建多模态消息
        multimodal_content = self._build_multimodal_message(question, image)
        eval_text = self._build_eval_prompt(completion)

        # 添加评估文本到消息中
        multimodal_content.append({"type": "text", "text": eval_text})
        ...
        try:
            async with session.post(
                    f'{self.api_base}/chat/completions', json=payload,
                    timeout=aiohttp.ClientTimeout(total=180)) as resp:  # 多模态请求增加超时时间
                if resp.status != 200:
                    error_text = await resp.text()
                    logger.warning(f'API error {resp.status}: {error_text[:200]}')
                    return 0.0
    ...
    async def __call__(self, completions: List[str], messages: List[List[dict]],
                       images: Optional[List[Union[str, List[str]]]] = None, **kwargs) -> List[float]:
        """
        使用多模态生成式奖励模型评估回答质量

        Args:
            completions: 模型生成的回答列表
            messages: 对话消息列表（用于提取用户问题）
            images: 超声图像列表，每个元素可以是base64字符串、图像URL或它们的列表（支持多张图像）
                    如果为None，则尝试从messages中提取image字段
            **kwargs: 额外参数
        ...
        """
        import aiohttp
        logger.info(f'AsyncUltrasoundGenRMReward initialized with model: {kwargs}')
        # 从messages中提取问题
        questions = []
        for msg_list in messages:
            question = ''
            ...
            for msg_list in messages:
                ...
                for msg in msg_list:
                    if msg.get('role') == 'user' and 'image' in msg:
                        img_list.append(msg['image'])
                images.append(img_list if img_list else '')
        else:
            # 确保images长度与completions一致
            if len(images) != len(completions):
                raise ValueError(
                    f"Number of images ({len(images)}) must match number of completions ({len(completions)})")

        # 并行调用API
        async with aiohttp.ClientSession() as session:
            tasks = []
            for q, img, c in zip(questions, images, completions):
                if not img:
                    logger.warning('No image provided for evaluation, returning 0.0 score')
                    tasks.append(asyncio.sleep(0, result=0.0))
                else:
                    tasks.append(self._score_single(session, q, img, c))
...
# 注册ORM
orms['async_ultrasound_genrm'] = AsyncUltrasoundGenRMReward